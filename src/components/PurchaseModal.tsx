...
import { 
  X, ChevronLeft, ChevronRight, Film, Calendar, Clock, 
  Armchair, Candy, Receipt, Loader2, CheckCircle, Plus, Minus 
} from "lucide-react";
...
interface PurchaseModalProps {
  // We handle visibility internally via window events, but can also accept props
}
...
export default function PurchaseModal() {
  ...
  useEffect(() => {
    const handleOpen = (e: Event) => {
      ...
      setTicketQuantities({ general: 0, premium: 0, popular: 0 });
      setSelectedSeats([]);
      setComboQuantities({ solo: 0, pareja: 0, familiar: 0, kids: 0 });
      setCustomerName("");
      setCustomerEmail("");
      
      // Pre-select movie if passed
      if (detail?.movieId && movies.length > 0) {
        const found = movies.find(m => m.id === detail.movieId);
        if (found) setSelectedMovie(found);
      } else {
        setSelectedMovie(null);
      }

      // Pre-select ticket type if passed
      if (detail?.ticketType) {
        setTicketQuantities(prev => ({
          ...prev,
          [detail.ticketType]: 1
        }));
        setStep(1); // Still start at step 1 to choose movie/showtime first
      }
    };

    window.addEventListener("open-purchase", handleOpen);
    return () => window.removeEventListener("open-purchase", handleOpen);
  }, [movies]);
  ...
  const toggleSeat = (seatLabel: string) => {
    ...
    setSelectedSeats(prev => {
      ...
      if (prev.length >= totalTickets) {
        // Replace oldest selection if limit is reached
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      if (conflict) {
        toast.error(`El asiento ${conflict} ya fue reservado por otra persona. Por favor, selecciona otro.`);
      ...
      // Create booking receipt summary
      const reservationCode = Math.random().toString(36).substring(2, 8).toUpperCase();
      
      toast.success("¡Compra realizada con éxito! Disfruta la función.");
      
      // Delay closing slightly so user sees success state
      setTimeout(() => {
        setIsOpen(false);
        setIsSubmitting(false);
        
        // Show success alert dialog with receipt details
        alert(
          `🎟️ ¡RESERVA CONFIRMADA!\n\n` +
          `Código de Reserva: ${reservationCode}\n` +
          `Película: ${selectedMovie?.title}\n` +
          `Sala: ${selectedShowtime?.room_name}\n` +
          `Fecha: ${selectedShowtime?.showing_date}\n` +
          `Horario: ${selectedShowtime?.showing_time?.slice(0, 5)}hs\n` +
          `Asientos: ${selectedSeats.join(", ")}\n` +
          `Monto Total: $${totalPrice.toLocaleString("es-AR")}\n\n` +
          `Presenta este código en boletería o al ingresar a la sala.`
        );
      }, 1000);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-4xl p-0 overflow-hidden bg-background/40 backdrop-blur-2xl border border-white/10 shadow-2xl h-[90vh] md:h-[80vh] flex flex-col rounded-2xl">
        
        {/* Header */}
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-black/40 relative">
        ...
        {/* Steps Bar indicator */}
        <div className="bg-black/20 px-6 py-2 flex items-center justify-between border-b border-white/5 gap-1.5 text-[10px] md:text-xs font-semibold tracking-wider uppercase text-muted-foreground">
        ...
        {/* Body Content */}
        <div className="flex-1 overflow-y-auto p-6 bg-gradient-to-b from-transparent to-black/10">
              ...
              >
                {isLoadingMovies ? (
                  <div className="flex flex-col items-center justify-center py-20">
                    ...
                    <p className="text-xs text-muted-foreground uppercase tracking-widest font-semibold">Cargando Cartelera...</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2.5 max-h-[45vh] overflow-y-auto pr-2">
                        {movies.map(movie => (
                          <div
                            key={movie.id}
                            onClick={() => {
                    ...
                    <div className="space-y-3">
                      ...
                      {selectedMovie ? (
                        selectedMovie.showtimes && selectedMovie.showtimes.length > 0 ? (
                          <div className="grid grid-cols-1 gap-2.5 max-h-[45vh] overflow-y-auto pr-2">
                          ...
                          <div className="h-[30vh] border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center p-6 text-center">
                            ...
                            <p className="text-sm text-muted-foreground">Esta película no tiene funciones activas hoy.</p>
                          </div>
                        )
                      ) : (
                        <div className="h-[40vh] border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center p-6 text-center">
                          <Film className="w-10 h-10 text-muted-foreground/30 mb-2 animate-pulse" />
                          <p className="text-sm text-muted-foreground">Por favor, selecciona una película a la izquierda primero para ver sus horarios.</p>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="max-w-xl mx-auto space-y-4">
                  ...
                  <div className="space-y-3">
                    {TICKET_TYPES.map(ticket => (
                      <div 
                        key={ticket.id} 
                        className="bg-card/40 border border-white/5 p-4 rounded-xl flex items-center justify-between transition-colors hover:border-white/10"
                      >
                ...
                <div className="flex flex-col items-center">
                  ...
                  {isLoadingSeats ? (
                    <div className="py-12 flex flex-col items-center justify-center">
                      ...
                      <p className="text-xs text-muted-foreground">Cargando plano de la sala...</p>
                    </div>
                  ) : (
              ...
              >
                <div className="space-y-4 max-w-xl mx-auto">
                  ...
                  <p className="text-xs text-muted-foreground text-center mb-6">
                    Añade combos de pochoclos y bebidas para disfrutar de tu función.
                  </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto items-start">
                  ...
                  <div className="space-y-4 bg-black/10 p-5 rounded-xl border border-white/5">
                    ...
                    <div className="space-y-3.5">
                      <div>
                        <label className="text-xs uppercase tracking-wider text-muted-foreground block mb-1">Nombre Completo</label>
                        <input
                          type="text"
                          required
                          value={customerName}
                          ...
                          className="w-full bg-black/40 border border-white/10 px-4 py-2.5 rounded-lg text-sm focus:border-primary focus:outline-none transition-colors"
                        />
                      </div>
                      <div>
                        <label className="text-xs uppercase tracking-wider text-muted-foreground block mb-1">Correo Electrónico</label>
                        <input
                          type="email"
                          required
                          value={customerEmail}
                  ...
                  {/* Black ticket aesthetic summary */}
                  <div className="bg-neutral-950 text-white rounded-xl border border-neutral-800 shadow-2xl relative overflow-hidden flex flex-col p-6 font-mono text-xs uppercase">
                    ...
                    <div className="text-center pb-4 border-b border-dashed border-neutral-800">
                      <h4 className="font-bold text-base tracking-wider text-primary">AMELIX CINEMA</h4>
                      <p className="text-[9px] text-neutral-500 mt-0.5">San Rafael, Mendoza</p>
                    </div>

                    {/* Movie Info */}
                    <div className="py-4 space-y-1.5 border-b border-dashed border-neutral-800">
                      <p className="font-bold text-sm text-neutral-100">{selectedMovie?.title}</p>
                      <div className="grid grid-cols-2 gap-2 text-[10px] text-neutral-400 mt-1">
                        <div>
                    ...
                    <div className="py-4 flex justify-between items-center text-sm">
                      <span className="font-bold text-neutral-400">TOTAL A PAGAR</span>
                      <span className="font-bold text-lg text-primary">${totalPrice.toLocaleString("es-AR")}</span>
                    </div>
        ...
        {/* Footer Actions */}
        <div className="p-5 border-t border-white/5 flex justify-between items-center bg-black/40">
          <div>
            {step > 1 && (
              <button
                onClick={prevStep}
